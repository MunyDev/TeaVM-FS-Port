...
public class FileSystemRandomAccessFile  implements DataOutput, DataInput, Closeable {
    ...
    private double filePointer= 0;
    private double fileSize = 0;
    private static final int O_RDONLY = 1;
    private static final int O_RDWR =   2;
    private static final int O_SYNC =   4;
    private static final int O_DSYNC =  8;
    private Uint8Array currentData;
    private double size;
    /**
     ...
     */
    FileSystem fs = null;
    public FileSystemRandomAccessFile(String name, String mode)
        
    {
       
       fs = FileSystem.getInstance(()->{
    	   fs.retrieveData(new FSFile(name), (ab, s)->{
    		   this.size = ab.getByteLength();
    		   this.currentData = Uint8Array.create(ab);
    	   });
       });
    }
     ...
     * @spec JSR-51
     */
    public FileSystemRandomAccessFile(File file, String mode)
...
//        open(name, imode);
     ...
     */
    public int read() {
    	return currentData.get((int) filePointer++);
    }
    ...
    public int read(byte b[], int off, int len) {
    	int foff = (int) filePointer;
    	int f  = 0;
    	for (int s = off; s < len+off; s++) {
    		b[s] = (byte) currentData.get(s-off+foff);
    		f++;
    	}
		return f;
    ...
    public final void readFully(byte b[], int off, int len) {
        ...
        for (int i = off; i < off+ len; i++) {
        	if (foff + i - off < fsize) {
        		...
        		b[i] = (byte) currentData.get(foff+i-off);
        	}
        }
    }
...
//    	int fsize = (int) size;
    ...
    public void setLength(double newLength) {
    	Uint8Array original = currentData;
    	currentData = Uint8Array.create(ArrayBuffer.create((int) newLength));
    	currentData.set(original);
     ...
     */
    public void close()  {
        fs.getInternalStream(fstr, false, (stream)->{
        	FileSystemNativeStream str = (FileSystemNativeStream)stream;
        	str.internal.write(currentData.getBuffer());
        	str.close();
        });
    }
